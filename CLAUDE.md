...
- **LIVE vs TEST env swap (2026-06-03).** Two importable env files in `.claude/` (gitignored): `hostinger.env` = LIVE; `test.env` = same 34 keys with the 6 testing deltas flipped (Turnstile TEST keys `1x...AA`, `MAX_SIGNUPS_PER_BUCKET=9999`, `LOGIN_MAX_FAILURES=9999`, `USER_DAILY_COST_MICROS_CAP=100000000`, `E2E_CHECKOUT_GEO_OVERRIDE_USER_ID=6b303c3b-…`). To run the gated E2E flows: Hostinger -> Env Vars -> **Import `test.env`** -> Save and redeploy. **REVERT after:** Import `hostinger.env` -> redeploy. Both carry identical keys so the swap is clean whether Hostinger merges or replaces. These WEAKEN protection while live — short windows only.
- **Cloudflare Email Address Obfuscation is OFF and must STAY off (2026-06-03).** Scrape Shield's email obfuscation rewrites literal emails at the edge + injects `email-decode.min.js` which mutates the DOM after React hydrates -> React #425 hydration mismatch (was firing on `/extract-emails-from-pdf`, the only page with example emails). Turning it off fixed it. If a hydration #425 reappears on an email-bearing page, check this toggle first.
- **Two test ACCOUNTS, by design (2026-06-03):** NON-admin = `durgapoja6408@gmail.com` (prod-e2e default; keeps the "non-admin blocked from /admin" SEV-0 test honest). ADMIN = `rajasekarjavaee+5@gmail.com` / `Cognizant@2026` — a Gmail +alias that NORMALIZES to `rajasekarjavaee@gmail.com` (∈ ADMIN_EMAILS) so it reads as admin, AND has a password so it's automatable (the bare admin account is Google-only). CI secrets: `PROD_E2E_TEST_*` (non-admin), `PROD_E2E_ADMIN_*`+`PROD_E2E_ADMIN_OK` (admin), `PROD_E2E_SIGNUP_OK` (enables the junk-account-creating signup spec). The prod-e2e **`payments` phase now runs EVERYTHING** incl. `admin-flow.spec.ts` (READ-ONLY /admin/* load checks) + `signup-flow.spec.ts` (creates a throwaway `playwright.e2e.*@example.com` account/run — see the spec's cleanup query). Admin creds in CI are higher-stakes; the admin spec is read-only + gated.
